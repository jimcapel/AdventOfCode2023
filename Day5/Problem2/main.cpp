...
int main()
{
    ...
    vector<long long> iWishThisWasTheInput;
    ...
    if (file.is_open())
    {
        ...
        while (getline(file, current))
        {
            ...
            if (lineIndex == 1)
            {
                for (int i = 0; i < iWishThisWasTheInput.size(); i++)
                {
                    if (i % 2 != 0)
                    {
                        for (long long x = iWishThisWasTheInput[i - 1]; x < iWishThisWasTheInput[i] + iWishThisWasTheInput[i - 1]; x++)
                        {
                            cout << x << endl;
                            numbers.push_back(x);
                        }
            ...
            for (int i = 0; i < current.length(); i++)
            {
                ...
                if ((asciiValue == 0x20 || i == (current.length() - 1)) && !number.empty())
                {
                    if (lineIndex == 0)
                    {
                        iWishThisWasTheInput.push_back(stoll(number));
                    }
                    else
                    {
